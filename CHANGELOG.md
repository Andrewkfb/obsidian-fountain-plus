...
- **Final Draft interop**: New `Export to Final Draft (.fdx)` and `Import from Final Draft (.fdx)` commands, so a script can leave the vault for a collaborator and come back. Export never overwrites an existing `.fdx` — it writes `script 1.fdx` instead, because a file with that name is as likely to be theirs as yours. Sections, synopses and notes are deliberately not exported: they're your outline and annotations, and the `.fdx` is what you send someone. On import, headings, cues and transitions that Fountain wouldn't recognise unaided are force-marked so they survive as the right element type.
- **Script statistics**: A new `Script statistics` command reports page count, scenes, sections, action paragraphs, dialogue blocks, and per-character speeches, lines and words. The page count is produced by paginating the script exactly as the PDF export does rather than estimating from length, so it's the number you'd get from printing it.
- **Touch support**: Index cards can be reordered by dragging the grip on a tablet. Reordering used the HTML5 drag-and-drop API, which WebKit never fires from touch, so on an iPad the grip simply did nothing; a pointer-event path now runs the same gesture into the same edit. The `+` / `#` insertion gutters and `+ section` bars are also reachable: they sit at `opacity: 0` until hovered, and a touch device produces no hover, so they were invisible rather than merely awkward.
- **Commands for keyboard-only actions**: Toggle edit mode, search, and move/copy selection to snippets now have command-palette entries. Their shortcuts are registered on the view (so `Mod+F` doesn't collide with Obsidian's own search), which left them unreachable without a hardware keyboard.
- **Settings**: A settings tab remembers PDF export defaults — paper size, bold scene headings, and the notes/synopsis/margin-mark choices — instead of making you re-pick them on every export, plus whether the editor starts with spell check on.
- **Fix**: The PDF no longer silently drops title page keys it has no placement rule for. `Copyright`, `Revision` and any key you invented rendered in the reading view but vanished from the PDF; they now print in the lower-left block, labelled with their key.
- **Fix**: Multi-line title page values printed upside down in the PDF. The lower-left and lower-right blocks are laid out from the bottom of the page upward, and emitting lines in reading order while stepping upward reversed them — a two-line `Contact` came out with its second line above its first.
- **Fix**: Typing `#` at the end of a document no longer throws. `Section` required at least one character after the `#`s, so a bare `#` — the first keystroke of every section heading typed at the end of a script — matched no rule at all and the parse threw, from inside a CodeMirror `StateField` update. Verified against 40,000 fuzzed documents that the grammar fix changes *no* existing parse: it only accepts input that previously threw.
- **Fix**: Parse failures are now survivable everywhere. The code guarded against them with `if ("error" in script)` in eight places, but `parse()` returns a `FountainScript` or throws and never returns an error object, so all eight guards were dead code and real failures propagated as exceptions. A new `parseFountain()` makes the parser total the way `readonly_view_state.ts` always claimed it was — a document the grammar rejects comes back as action lines, so the writer still sees their text — and the dead guards are gone.
- **Performance**: The PDF library, which is ~78% of the plugin bundle, no longer initialises when the plugin loads. It is imported on first export instead, moving 61% of the bundle off the startup path (it was 1%).
- **Performance**: The cross-file link index no longer parses every `.fountain` file in the vault on startup, and on every file create or delete. A file with no `[[>` in it cannot contain a link, so it is skipped without parsing.
- **Performance**: `withHiddenElementsRemoved` is memoized per settings combination, so toggling notes/synopsis/boneyard or simply re-rendering the readonly view no longer rebuilds a filtered script (and re-runs dual pairing and the character scan) each time.
- **Internal**: CI, at last — `.github/workflows/ci.yml` runs lint, typecheck, unit tests and build, plus the e2e suite under `xvfb`. The e2e job asserts that specs actually ran: the wdio runner can exit 0 having run nothing at all, which is precisely what it had been doing.
- **Internal**: Biome is now configured in-repo (`biome.json`) with `npm run lint`, rather than depending on each contributor's editor settings. The formatter is deliberately not yet applied repo-wide.
- **Scene heading autocomplete**: Typing a scene heading now completes the slugline (`INT.`, `EXT.`, `INT./EXT.`, …) and then the locations already used in the script, most recently used first. Reusing a location verbatim matters beyond keystrokes: headings are matched as strings by the index cards, the outline and every downstream tool, so two spellings of one place quietly become two places.
- **Internal**: `npm install` works on a clean clone again. `@codemirror/state` was floating on `^6.5.4` while `obsidian` pins the peer to exactly `6.5.0`, so a fresh `npm install` or `npm ci` failed with `ERESOLVE`.
- **Internal**: The grammar change was validated by diffing full ASTs (ranges included) against the pre-change parser over the demo corpus plus 40,000 randomized mutations — the fast path is behaviour-preserving, not just test-passing. Its boundary conditions are pinned by `__tests__/plain_text_run.test.ts`: blanks before a boneyard or a centered line's `<` belong to *those* elements' ranges, so the run has to stop short of them. Title page behaviour is covered by `__tests__/title_page.test.ts` and `test/e2e/specs/title_page.e2e.ts`.
